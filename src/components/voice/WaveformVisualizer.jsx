...
function WaveformVisualizer({ audioBuffer, onSelection, onEdit }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.error('waveform_load_failed', { duration: audioBuffer?.duration, sampleRate: audioBuffer?.sampleRate }, error)
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.error('waveform_render_failed', { zoom, selection, dataPoints: waveformData.length }, error)
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (duration > 0.1) { // Minimum 100ms selection
  ...
  const handleCut = useCallback(() => {
    try {
      ...
      logger.error('waveform_cut_failed', { selection, zoom, duration: audioBuffer?.duration }, error)
