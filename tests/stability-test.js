...
try {
  ...
  if (serverCode.includes('setInterval(() => {\n    sseClients.forEach') || 
...
try {
  ...
  if (authMatches && authMatches.length >= 2) {
    ...
    console.log(`⚠️  WARN: Only ${authMatches ? authMatches.length : 0} endpoints use authenticateToken`);
  }
...
try {
  ...
  if (loggerCode.includes('finally {\n    this.isSending = false;') || 
